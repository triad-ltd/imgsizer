...
$plugin_info = array( // EE2 support
	...
	'pi_description' => 'Image Resizer - resizes/caches/makes placeholders',
...
class Imgsizer {
	...
	function size() {
		...
		$this->output['path'] = reduce_double_slashes($this->settings['cache_path'] . '/sized/' . $inf['dirname'] . '/');
		$this->output['filename'] = $this->settings['width'].'-'.$this->settings['height'].'-'.$this->settings['quality'].'-'.$inf['basename'];
		$this->output['url'] = reduce_double_slashes($this->settings['cache_url'] . '/sized/' . $inf['dirname'] . '/' . $this->output['filename']);
		if (!is_dir($this->output['path'])) {
			if (!mkdir($this->output['path'], 0777, true)) {
				return $this->error("Unable to create placeholder cache folder");
			}
